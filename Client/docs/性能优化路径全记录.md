...
![稳定战斗 Timeline](../DOTS/稳定战斗Timeline（初）.png)
...
![Job引入后峰值 Timeline](../DOTS/Job引入后峰值Timelie.png)
...
![Job引入后峰值层级](../DOTS/Job引入后峰值层级.png)
...
![Job引入后峰值 JobWorker](../DOTS/Job引入后峰值Jobworker.png)
...
![最后 Timeline](../DOTS/最后Timeline.png)
...
| 图片 | 阶段 | 说明 |
|------|------|------|
| `../DOTS/稳定战斗Timeline（初）.png` | Phase 0 | 初始 Timeline：54ms 帧，主线程阻塞 |
| `../DOTS/稳定战斗层级(初）.png` | Phase 0 | 初始层级：HandleUnits 73.6%，GC.Collect 7.4% |
| `../DOTS/稳定战斗内存（初）.png` | Phase 0 | 初始内存：42ms GC 暂停，托管堆 38.6 MB |
| `../DOTS/Brsut优化Timeline.png` | Phase 1 | Burst 后 Timeline：GC 下降，主线程仍 49ms |
| `../DOTS/Brust优化层级（GC明显下降）.png` | Phase 1 | Burst 后层级：GC 分配降至 33.8 KB |
| `../DOTS/Job引入后峰值Timelie.png` | Phase 2 | Job 后 Timeline：32ms 帧，Worker Thread 活跃 |
| `../DOTS/Job引入后峰值层级.png` | Phase 2 | Job 后层级：AStarBurstJob 8.7%，FindTargets 63.3% |
| `../DOTS/Job引入后峰值Jobworker.png` | Phase 2 | Job Worker：AStarBurstJob 在 Worker 线程执行 |
| `../DOTS/最后Timeline.png` | Phase 3 | 最终 Timeline：帧时间 8-16ms 稳定 60+ FPS |
| `../DOTS/最后层级.png` | Phase 3 | 最终层级：GC 分配 0 B，ExecuteFrame 11.8% |
| `../DOTS/最后内存.png` | Phase 3 | 最终内存：托管堆 28.4 MB，无 GC 尖峰 |

---

> 相关文档：
> - [Burst 改造阶段总结](changelog/2026-06-09_Burst改造阶段总结.md)
> - [Jobs 多线程优化](changelog/2026-06-07_Jobs多线程优化.md)
> - [预算型微批处理实施](changelog/2026-06-12_预算型微批处理实施.md)
> - [ECS 引入](changelog/2026-06-08_ECS引入.md)
> - [投射物对象池](changelog/2026-06-18_投射物对象池.md)
