namespace DevelopersHub.ClashOfWhatecer
{
    ...
    public class Player : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            RealtimeNetworking.OnConnectingToServerResult += ConnectingToServerResult;
        ...
        private void ConnectingToServerResult(bool successful)
        {
            if (successful)
            {
                SendAuthRequest();
            ...
            {
                Debug.LogWarning("Unable to connect to server, auth request was not sent.");
            }
        }
                ...
                switch ((RequestsID)id)
                {
                    ...
                    case RequestsID.SYNC:
                        ...
                        if (response == 1)
                        {
                            ...
                            // GZip 解压 + XML 反序列化在线程池内完成，不阅塞主线程。
                            ProcessSyncAsync(playerBytes);
                        }
                        else
                        {
                            updating = false;
        ...
        // GZip 解压和 XML 反序列化在线程池内完成；后续的 SyncData() 由 Unity
        // SynchronizationContext 调度回主线程执行，安全操作所有 Unity API。
        private async void ProcessSyncAsync(byte[] bytes)
        {
            try
            {
                string xml = await Data.DecompressAsync(bytes);
                Data.Player playerSyncData = await Data.DesrializeAsync<Data.Player>(xml);
                // 以下代码回到 Unity 主线程执行
                ...
                if (playerSyncData.banned)
                {
                    ...
                    return; // 封号时不重置 updating，与原始逻辑一致
                }
            ...
            {
                Debug.LogError("[Player] SYNC data processing failed: " + e.Message);
                updating = false;
            }
        }
        ...
        public void SyncData(Data.Player player)
        {
            ...
            */
            ...
            string usernameText = Data.DecodeString(data.name);
            if (UI_Main.instanse._usernameText.text != usernameText)
            {
                UI_Main.instanse._usernameText.text = usernameText;
                UI_Main.instanse._usernameText.ForceMeshUpdate(true);
            }

            string trophiesText = data.trophies.ToString();
            if (UI_Main.instanse._trophiesText.text != trophiesText)
            {
                UI_Main.instanse._trophiesText.text = trophiesText;
                UI_Main.instanse._trophiesText.ForceMeshUpdate(true);
            }

            string levelText = data.level.ToString();
            if (UI_Main.instanse._levelText.text != levelText)
            {
                UI_Main.instanse._levelText.text = levelText;
                UI_Main.instanse._levelText.ForceMeshUpdate(true);
            }

            string xpText = data.xp.ToString();
            if (UI_Main.instanse._xpText.text != xpText)
            {
                UI_Main.instanse._xpText.text = xpText;
                UI_Main.instanse._xpText.ForceMeshUpdate(true);
            }
            
            int reqXp = Data.GetNexLevelRequiredXp(data.level);
            float xpFillAmount = (reqXp > 0 ? ((float)data.xp / (float)reqXp) : 0);
            if (!Mathf.Approximately(UI_Main.instanse._xpBar.fillAmount, xpFillAmount))
            {
                UI_Main.instanse._xpBar.fillAmount = xpFillAmount;
            }
