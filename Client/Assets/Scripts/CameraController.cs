namespace DevelopersHub.ClashOfWhatecer
{
    ...
    public class CameraController : MonoBehaviour
    {
        ...
        private int _zoomVersion = 0; public int zoomVersion { get { return _zoomVersion; } }
        private int _viewportVersion = 0; public int viewportVersion { get { return _viewportVersion; } }
        ...
        [SerializeField] private float _battleLongPressDelay = 0.35f;
        [SerializeField] private float _battleContinuousDeployInterval = 0.08f;
        [SerializeField] private float _battleContinuousDeployMoveTolerance = 12f;
        private bool _battleDeployPointerHeld = false;
        private bool _battleSuppressNextClick = false;
        private float _battleNextDeployTime = 0f;
        private Vector2 _battleDeployStartScreenPosition = Vector2.zero;
        ...
        private Vector3 _lastPlanCameraPosition = Vector3.zero;
        private Quaternion _lastPlanCameraRotation = Quaternion.identity;
        private float _lastPlanOrthographicSize = -1;
        private int _lastPlanScreenWidth = -1;
        private int _lastPlanScreenHeight = -1;
        ...
        public void Initialize()
        {
            ...
            UpdatePlanBounds(true);
        }

        private void OnEnable()
        {
            inputs.Enable();
            inputs.Main.Move.started += OnMoveStarted;
            inputs.Main.Move.canceled += OnMoveCanceled;
            inputs.Main.TouchZoom.started += OnTouchZoomStarted;
            inputs.Main.TouchZoom.canceled += OnTouchZoomCanceled;
            inputs.Main.PointerClick.performed += OnPointerClickPerformed;
        }
        ...
        private void OnDisable()
        {
            ...
            inputs.Main.PointerClick.performed -= OnPointerClickPerformed;
            inputs.Disable();
            CancelBattleDeployHold();
            _battleSuppressNextClick = false;
        }

        private void OnMoveStarted(InputAction.CallbackContext context)
        {
            BeginBattleDeployHold();
            MoveStarted();
        }
        ...
        private void OnTouchZoomStarted(InputAction.CallbackContext context)
        {
            CancelBattleDeployHold();
            ZoomStarted();
        }

        private void OnTouchZoomCanceled(InputAction.CallbackContext context)
        {
            ZoomCanceled();
        }
        ...
        private void ScreenClicked()
        {
            ...
            {
                if (_battleSuppressNextClick)
                {
                    _battleSuppressNextClick = false;
                    return;
                }

                TryPlaceBattleItemAtScreenPosition(position, results.Count > 0, false);
            }
        ...
        private void BeginBattleDeployHold()
        {
            _battleSuppressNextClick = false;
            CancelBattleDeployHold();

            if (!UI_Battle.instanse.isActive || UI_Battle.instanse.selectedUnit < 0 || UI_Battle.instanse.selectedSpell >= 0 || _zooming)
            {
                return;
            }

            Vector2 position = inputs.Main.PointerPosition.ReadValue<Vector2>();
            if (IsScreenPointOverUI(position))
            {
                return;
            }

            _battleDeployPointerHeld = true;
            _battleDeployStartScreenPosition = position;
            _battleNextDeployTime = Time.unscaledTime + _battleLongPressDelay;
        }

        private void CancelBattleDeployHold()
        {
            _battleDeployPointerHeld = false;
            _battleNextDeployTime = 0f;
        }

        private bool TryPlaceBattleItemAtScreenPosition(Vector2 position, bool overUI, bool unitsOnly)
        {
            if (!UI_Battle.instanse.isActive || overUI)
            {
                return false;
            }
            ...
            if (unitsOnly)
            {
                if (UI_Battle.instanse.selectedUnit < 0 || UI_Battle.instanse.selectedSpell >= 0)
                {
                    return false;
                }
            }
            else if (UI_Battle.instanse.selectedUnit < 0 && UI_Battle.instanse.selectedSpell < 0)
            {
                return false;
            }
            ...
            return UI_Battle.instanse.PlaceUnit(Mathf.FloorToInt(gridPos.x), Mathf.FloorToInt(gridPos.y));
        }

        private void ProcessBattleContinuousDeploy()
        {
            if (!_battleDeployPointerHeld)
            {
                return;
            }

            if (_zooming || !UI_Battle.instanse.isActive || UI_Battle.instanse.selectedUnit < 0 || UI_Battle.instanse.selectedSpell >= 0)
            {
                CancelBattleDeployHold();
                return;
            }
            ...
            Vector2 position = inputs.Main.PointerPosition.ReadValue<Vector2>();
            if ((position - _battleDeployStartScreenPosition).sqrMagnitude > _battleContinuousDeployMoveTolerance * _battleContinuousDeployMoveTolerance)
            {
                CancelBattleDeployHold();
                return;
            }

            if (Time.unscaledTime < _battleNextDeployTime)
            {
                return;
            }

            if (TryPlaceBattleItemAtScreenPosition(position, IsScreenPointOverUI(position), true))
            {
                _battleSuppressNextClick = true;
            }
            _battleNextDeployTime = Time.unscaledTime + _battleContinuousDeployInterval;
        }
        ...
        private void UpdatePlanBounds(bool force = false)
        {
            bool zoomChanged = force || Mathf.Abs(_camera.orthographicSize - _lastPlanOrthographicSize) >= 0.001f || Screen.width != _lastPlanScreenWidth || Screen.height != _lastPlanScreenHeight;
            bool viewportChanged = force || zoomChanged || _camera.transform.position != _lastPlanCameraPosition || _camera.transform.rotation != _lastPlanCameraRotation;
            ...
            _lastPlanCameraPosition = _camera.transform.position;
            _lastPlanCameraRotation = _camera.transform.rotation;
            _lastPlanOrthographicSize = _camera.orthographicSize;
            _lastPlanScreenWidth = Screen.width;
            _lastPlanScreenHeight = Screen.height;
