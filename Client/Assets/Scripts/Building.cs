namespace DevelopersHub.ClashOfWhatecer
{
    ...
    public class Building : MonoBehaviour
    {
        ...
        private int _lastBuildBarRemainingSeconds = int.MinValue;
        ...
        private void OnViewportChanged() => UpdateUIPositions();

        private void Update()
        {
            // 采集按钮超时倒计时
            if (collectButton != null && !scout && collectTimeout)
            {
                if (collectTimer > 0)
            ...
            // 施工完成检测 + 进度文字按秒刷新
            if (_data.isConstructing && _data.buildTime > 0)
            {
                ...
                {
                    _data.isConstructing = false;
                    isCons = false;
                    level = level + 1;
                    lastChange = System.DateTime.Now;
                    _lastBuildBarRemainingSeconds = int.MinValue;
                    AdjustUI(true);
                    Player.instanse.RushSyncRequest();
                }
                else if (buildBar != null && buildBar.gameObject.activeSelf)
                {
                    int remaining = Mathf.Max(0, Mathf.FloorToInt((float)span.TotalSeconds));
                    if (_lastBuildBarRemainingSeconds != remaining)
                    {
                        buildBar.texts[0].text = Tools.SecondsToTimeFormat(span);
                        buildBar.texts[0].ForceMeshUpdate(true);
                        _lastBuildBarRemainingSeconds = remaining;
                    }
...
/*
        ...
        // 仅负责 SetActive / 文字可见性，不做坐标计算。由 BuildGrid.RefreshBuildings、
        // DataSynced、collectTimeout 到期等状态变化时调用，彻底消除每帧 SetActive 开销。
        private void RefreshVisibility()
        {
            if (collectButton != null && !scout)
            {
                bool show = false;
                if (!collectTimeout)
                {
                    switch (id)
                    {
                        case Data.BuildingID.goldmine:
                            show = _data.goldStorage >= Data.minGoldCollect && !collecting
                                && !_data.isConstructing && Player.instanse.gold < Player.instanse.maxGold;
                            break;
                        case Data.BuildingID.elixirmine:
                            show = _data.elixirStorage >= Data.minElixirCollect && !collecting
                                && !_data.isConstructing && Player.instanse.elixir < Player.instanse.maxElixir;
                            break;
                        case Data.BuildingID.darkelixirmine:
                            show = _data.darkStorage >= Data.minDarkElixirCollect && !collecting
                                && !_data.isConstructing && Player.instanse.darkElixir < Player.instanse.maxDarkElixir;
                            break;
                    }
                }
                SetActiveIfChanged(collectButton.gameObject, show);
            }

            SetActiveIfChanged(_boostEffects, _boostEffects != null && _data.boost > Player.instanse.data.nowTime);
            ...
            if (isConstructing)
            {
                ...
                if (Player.instanse.data.nowTime < _data.constructionTime)
                    span = _data.constructionTime - Player.instanse.data.nowTime;
                showBar = span.TotalSeconds > 0;
            }

            if (buildBar != null && buildBar.gameObject.activeSelf != showBar)
            {
                buildBar.gameObject.SetActive(showBar);
                if (!showBar) _lastBuildBarRemainingSeconds = int.MinValue;
            }
            SetActiveIfChanged(_constructionEffects, isConstructing);
        }

        // 仅负责屏幕坐标映射，由 UIEvents.onViewportChanged 事件驱动（相机移动时触发）。
        // 只对当前可见的 UI 元素计算，避免为隐藏元素做无效运算。
        private void UpdateUIPositions()
        {
            if (CameraController.instanse == null || UI_Main.instanse == null || UI_Main.instanse._grid == null) return;
            ...
            if (collectButton != null && collectButton.gameObject.activeSelf)
            {
                ...
                collectButton.rect.anchoredPosition = new Vector2(
                    (end.x - planDownLeft.x) / w * Screen.width,
                    (end.y - planDownLeft.y) / h * Screen.height);
            }

            if (buildBar != null && buildBar.gameObject.activeSelf)
            {
                Vector3 end = UI_Main.instanse._grid.GetEndPosition(this);
                buildBar.rect.anchoredPosition = new Vector2(
                    (end.x - planDownLeft.x) / w * Screen.width,
                    (end.y - planDownLeft.y) / h * Screen.height);
            }
        }
