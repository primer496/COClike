...
{ 
    ...
    public class StartLoading : MonoBehaviour
    {
        ...
        [Header("Server")]
        [SerializeField] private GameObject _serverPanel = null;
        [SerializeField] private TMP_InputField _inputServerIP = null;
        [SerializeField] private TMP_InputField _inputServerPort = null;
        [SerializeField] private Button _buttonConnect = null;
        [SerializeField] private TextMeshProUGUI _textServerLog = null;
        ...
        private const string ServerIPPrefKey = "server_ip";
        private const string ServerPortPrefKey = "server_port";
        ...
        private void Start()
        {
            ...
            string savedIP = PlayerPrefs.GetString(ServerIPPrefKey, "127.0.0.1");
            int savedPort = PlayerPrefs.GetInt(ServerPortPrefKey, 5555);
            Client.instance.settings.SetIP(savedIP);
            Client.instance.settings.SetPort(savedPort);

            if (_serverPanel != null)
            {
                _serverPanel.gameObject.SetActive(true);
                if (_inputServerIP != null) _inputServerIP.text = savedIP;
                if (_inputServerPort != null) _inputServerPort.text = savedPort.ToString();
                if (_textServerLog != null) _textServerLog.text = "";
                if (_buttonConnect != null) _buttonConnect.onClick.AddListener(OnConnectClicked);
            }
            else
            {
        ...
        {
            string ip = (_inputServerIP != null && !string.IsNullOrEmpty(_inputServerIP.text.Trim()))
                ? _inputServerIP.text.Trim() : "127.0.0.1";
            int port = 5555;
            if (_inputServerPort != null && !string.IsNullOrEmpty(_inputServerPort.text.Trim()))
            {
                int.TryParse(_inputServerPort.text.Trim(), out port);
            }
            if (port <= 0 || port > 65535) port = 5555;

            PlayerPrefs.SetString(ServerIPPrefKey, ip);
            PlayerPrefs.SetInt(ServerPortPrefKey, port);
            Client.instance.settings.SetIP(ip);
            Client.instance.settings.SetPort(port);

            if (_serverPanel != null) _serverPanel.gameObject.SetActive(false);
            if (_textServerLog != null) _textServerLog.text = "";
        ...
        private void ConnectionResponse(bool successful)
        {
            ...
            else
            {
                ...
                {
                    _serverPanel.gameObject.SetActive(true);
                    if (_textServerLog != null)
                    {
                        switch (Language.instanse.language)
                        {
                            case Language.LanguageID.persian:
                                _textServerLog.text = "اتصال به سرور برقرار نشد. آدرس IP و پورت را بررسی کنید.";
                                break;
                            default:
                                _textServerLog.text = "Failed to connect. Please check the IP address and port, then try again.";
                                break;
                        }
                    }
                }
