...
namespace DevelopersHub.RealtimeNetworking.Server
{
    class Database
    {
        ...
        private const string _mysqlPassword = "adaafhff123A!";
        private const string _mysqlDatabase = "clash_of_whatever";
        private const float _defenseDamageMultiplier = 1f / 3f;
        private const int _defenseHealthMultiplier = 3;

        public static MySqlConnection GetMysqlConnection()
        {
            MySqlConnection connection = new MySqlConnection("SERVER=" + _mysqlServer + "; DATABASE=" + _mysqlDatabase + "; UID=" + _mysqlUsername + "; PWD=" + _mysqlPassword + "; POOLING=FALSE; CHARSET=UTF8; SSLMODE=NONE; AllowPublicKeyRetrieval=True");
        ...
        private static readonly HashSet<string> _loggedUnsupportedBuildingIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

        private static bool TryParseBuildingID(string globalId, out Data.BuildingID buildingId)
        {
            if (Enum.TryParse(globalId, true, out buildingId))
            {
                return true;
            }

            buildingId = Data.BuildingID.townhall;
            if (String.IsNullOrWhiteSpace(globalId))
            {
                return false;
            }

            lock (_loggedUnsupportedBuildingIds)
            {
                if (_loggedUnsupportedBuildingIds.Add(globalId))
                {
                    Console.WriteLine($"Skipping unsupported building id '{globalId}'.");
                }
            }
        ...
        private static bool IsDefensiveBuilding(Data.Building building)
        {
            return building != null && building.targetType != Data.BuildingTargetType.none;
        }

        private static void ApplyDefensiveBuildingBalance(Data.Building building)
        {
            if (!IsDefensiveBuilding(building))
            {
                return;
            }

            building.damage *= _defenseDamageMultiplier;
            building.health *= _defenseHealthMultiplier;
        }
        ...
        public static void UpgradeExistingAccountsToTownHall(int targetTownHallLevel)
        {
            Data.BuildingAvailability limits = Data.GetTownHallLimits(targetTownHallLevel);
            if (limits == null || limits.buildings == null || limits.buildings.Length == 0)
            {
                throw new InvalidOperationException($"Town hall level {targetTownHallLevel} configuration was not found.");
            }

            using (MySqlConnection connection = GetMysqlConnection())
            {
                List<long> accountIds = GetAllAccountIds(connection);
                List<MigrationBuildingDefinition> buildingDefinitions = GetMigrationBuildingDefinitions(connection, targetTownHallLevel, limits);
                Console.WriteLine($"Preparing TH{targetTownHallLevel} migration for {accountIds.Count} existing accounts.");

                int migratedCount = 0;
                foreach (long accountId in accountIds)
                {
                    RebuildVillageForAccount(connection, accountId, buildingDefinitions);
                    migratedCount++;
                }

                connection.Close();
                Console.WriteLine($"TH{targetTownHallLevel} migration complete. Updated {migratedCount} accounts.");
            }
        }

        /// <summary>
        /// 一键将所有玩家：防御建筑等级设为 1、兵种等级设为满级、法术等级设为满级。
        /// 直接操作 MySQL 的 buildings / units / spells 三张表，
        /// 以 server_buildings.target_type、server_units、server_spells 为权威参照。
        /// </summary>
        public static void UpgradeAllToMax()
        {
            using (MySqlConnection connection = GetMysqlConnection())
            {
                ...
                string defQuery = @"
                    UPDATE buildings b
                    INNER JOIN server_buildings sb ON b.global_id = sb.global_id AND sb.level = 1
                    SET b.level = 1,
                        b.is_constructing = 0,
                        b.construction_time = NOW(),
                        b.construction_build_time = 0
                    WHERE sb.target_type != 'none'";
                using (MySqlCommand cmd = new MySqlCommand(defQuery, connection))
                {
                    buildingsChanged = cmd.ExecuteNonQuery();
                }

                // 2) 兵种 -> 各 global_id 在 server_units 中的最高等级，并设置为已训练、已准备
                string unitQuery = @"
                    UPDATE units u
                    INNER JOIN (
                        SELECT global_id, MAX(level) AS max_level
                        FROM server_units
                        GROUP BY global_id
                    ) su ON u.global_id = su.global_id
                    SET u.level = su.max_level,
                        u.trained = 1,
                        u.ready = 1,
                        u.trained_time = 0";
                using (MySqlCommand cmd = new MySqlCommand(unitQuery, connection))
                {
                    unitsChanged = cmd.ExecuteNonQuery();
                }

                // 3) 法术 -> 各 global_id 在 server_spells 中的最高等级，并设置为已酿造、已准备
                string spellQuery = @"
                    UPDATE spells s
                    INNER JOIN (
                        SELECT global_id, MAX(level) AS max_level
                        FROM server_spells
                        GROUP BY global_id
                    ) ss ON s.global_id = ss.global_id
                    SET s.level = ss.max_level,
                        s.brewed = 1,
                        s.ready = 1,
                        s.brewed_time = 0";
                using (MySqlCommand cmd = new MySqlCommand(spellQuery, connection))
                {
                    spellsChanged = cmd.ExecuteNonQuery();
                }

                connection.Close();
                Console.WriteLine($"--- db-upgrade-all 完成 ---");
                Console.WriteLine($"防御建筑设为 1 级：{buildingsChanged} 个");
                Console.WriteLine($"兵种设为满级：{unitsChanged} 个");
                Console.WriteLine($"法术设为满级：{spellsChanged} 个");
            }
        }

        private sealed class MigrationBuildingDefinition
        {
        ...
        private sealed class MigrationPlacement
        {
            public string GlobalId { get; set; } = string.Empty;
            public int Level { get; set; }
        ...
        private static List<long> GetAllAccountIds(MySqlConnection connection)
        {
            ...
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            if (long.TryParse(reader["id"].ToString(), out long accountId))
                            {
        ...
        private static List<MigrationBuildingDefinition> GetMigrationBuildingDefinitions(MySqlConnection connection, int targetTownHallLevel, Data.BuildingAvailability limits)
        {
            List<MigrationBuildingDefinition> definitions = new List<MigrationBuildingDefinition>();
            for (int i = 0; i < limits.buildings.Length; i++)
            {
                Data.BuildingCount limit = limits.buildings[i];
                if (limit == null || limit.count <= 0)
                {
                    continue;
                }

                if (!TryParseBuildingID(limit.id, out _))
                {
                    Console.WriteLine($"Skipping unsupported migration building '{limit.id}' because the current runtime does not support it.");
                    continue;
                }

                int desiredLevel = limit.id == Data.BuildingID.townhall.ToString() ? targetTownHallLevel : limit.maxLevel;
                int maxAvailableLevel = 0;
                string levelQuery = String.Format("SELECT MAX(level) AS max_level FROM server_buildings WHERE global_id = '{0}';", limit.id);
                using (MySqlCommand levelCommand = new MySqlCommand(levelQuery, connection))
                {
                    using (MySqlDataReader levelReader = levelCommand.ExecuteReader())
                    {
                        if (levelReader.HasRows)
                        {
                            while (levelReader.Read())
                            {
                                int.TryParse(levelReader["max_level"].ToString(), out maxAvailableLevel);
                            }
                        }
                    }
                }
                ...
                {
                    if (limit.id == Data.BuildingID.townhall.ToString())
                    {
                        throw new InvalidOperationException($"Missing server_buildings data for {limit.id}.");
                    }

                    Console.WriteLine($"Skipping unsupported building '{limit.id}' because server_buildings has no rows for it.");
                    continue;
                }

                if (desiredLevel > maxAvailableLevel)
                {
                    desiredLevel = maxAvailableLevel;
                }

                string query = String.Format("SELECT columns_count, rows_count FROM server_buildings WHERE global_id = '{0}' AND level = {1} LIMIT 1;", limit.id, desiredLevel);
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (!reader.HasRows)
                        {
                            throw new InvalidOperationException($"Missing server_buildings row for {limit.id} level {desiredLevel}.");
                        }

                        while (reader.Read())
                        {
                            definitions.Add(new MigrationBuildingDefinition
                            {
                                GlobalId = limit.id,
                                Level = desiredLevel,
                                Count = limit.count,
                                Width = int.Parse(reader["columns_count"].ToString()),
                                Height = int.Parse(reader["rows_count"].ToString())
                            });
                        }
                    }
                }
            ...
            return definitions
                .OrderByDescending(definition => definition.Width * definition.Height)
                .ThenByDescending(definition => definition.Count)
                .ThenBy(definition => definition.GlobalId)
                .ToList();
        }

        private static void RebuildVillageForAccount(MySqlConnection connection, long accountId, List<MigrationBuildingDefinition> buildingDefinitions)
        {
            List<MigrationPlacement> placements = GenerateVillagePlacements(buildingDefinitions);

            string query = String.Format("DELETE FROM buildings WHERE account_id = {0};", accountId);
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                command.ExecuteNonQuery();
            }

            string insertQuery = "INSERT INTO buildings (global_id, account_id, level, x_position, y_position, gold_storage, elixir_storage, dark_elixir_storage, boost, construction_time, is_constructing, construction_build_time, track_time, x_war, y_war) VALUES (@globalId, @accountId, @level, @x, @y, 0, 0, 0, NOW() - INTERVAL 1 HOUR, NOW() - INTERVAL 1 HOUR, 0, 0, NOW() - INTERVAL 1 HOUR, @warX, @warY);";
            foreach (MigrationPlacement placement in placements)
            {
                using (MySqlCommand command = new MySqlCommand(insertQuery, connection))
                {
                    command.Parameters.AddWithValue("@globalId", placement.GlobalId);
                    command.Parameters.AddWithValue("@accountId", accountId);
                    command.Parameters.AddWithValue("@level", placement.Level);
                    command.Parameters.AddWithValue("@x", placement.X);
                    command.Parameters.AddWithValue("@y", placement.Y);
                    command.Parameters.AddWithValue("@warX", placement.X);
                    command.Parameters.AddWithValue("@warY", placement.Y);
                    command.ExecuteNonQuery();
                }
            }

            FillVillageResources(connection, accountId);

            query = String.Format("UPDATE accounts SET gems = 50000 WHERE id = {0};", accountId);
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                command.ExecuteNonQuery();
            }
        ...
        private static List<MigrationPlacement> GenerateVillagePlacements(List<MigrationBuildingDefinition> buildingDefinitions)
        {
            bool[,] occupied = new bool[Data.gridSize, Data.gridSize];
            List<MigrationPlacement> placements = new List<MigrationPlacement>();

            for (int i = 0; i < buildingDefinitions.Count; i++)
            {
                MigrationBuildingDefinition definition = buildingDefinitions[i];
                for (int copyIndex = 0; copyIndex < definition.Count; copyIndex++)
                {
                    bool placed = false;
                    for (int y = 0; y <= Data.gridSize - definition.Height && !placed; y++)
                    {
                        for (int x = 0; x <= Data.gridSize - definition.Width; x++)
                        {
                            if (!CanPlaceBuilding(occupied, x, y, definition.Width, definition.Height))
                            {
                                continue;
                            }

                            MarkOccupied(occupied, x, y, definition.Width, definition.Height);
                            placements.Add(new MigrationPlacement
                            {
                    ...
                    if (!placed)
                    {
                        throw new InvalidOperationException($"Failed to place {definition.GlobalId} level {definition.Level} inside the {Data.gridSize}x{Data.gridSize} grid.");
                    }
                }
            }
        ...
        private static bool CanPlaceBuilding(bool[,] occupied, int startX, int startY, int width, int height)
        {
        ...
        private static void MarkOccupied(bool[,] occupied, int startX, int startY, int width, int height)
        {
            for (int x = startX; x < startX + width; x++)
            {
                for (int y = startY; y < startY + height; y++)
        ...
        private static void FillVillageResources(MySqlConnection connection, long accountId)
        {
            string query = String.Format(@"UPDATE buildings AS b
LEFT JOIN server_buildings AS s ON b.global_id = s.global_id AND b.level = s.level
SET
    b.gold_storage = CASE WHEN b.global_id IN ('townhall', 'goldstorage', 'goldmine') THEN s.gold_capacity ELSE b.gold_storage END,
    b.elixir_storage = CASE WHEN b.global_id IN ('townhall', 'elixirstorage', 'elixirmine') THEN s.elixir_capacity ELSE b.elixir_storage END,
    b.dark_elixir_storage = CASE WHEN b.global_id IN ('townhall', 'darkelixirstorage', 'darkelixirmine') THEN s.dark_elixir_capacity ELSE b.dark_elixir_storage END
WHERE b.account_id = {0};", accountId);
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                command.ExecuteNonQuery();
            }
        ...
        private static Data.InitializationData _AuthenticatePlayerAsync(int id, string device, string password, string username)
        {
            ...
            using (MySqlConnection connection = GetMysqlConnection())
            {
                // Auth by device_id only — device_id is already unique per editor instance (via GetDeviceID).
                // Password is no longer used for lookup to avoid new-account-on-mismatch issues.
                string query = String.Format("SELECT id, password, is_online, client_id FROM accounts WHERE device_id = '{0}'", device);
                bool found = false;
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ...
                                initializationData.password = reader["password"].ToString();
                                found = true;
                            }
                        }
                    }
                ...
                if (found == false)
                {
                    ...
                    query = String.Format("INSERT INTO buildings (global_id, account_id, x_position, y_position, level, track_time, x_war, y_war) VALUES('{0}', {1}, {2}, {3}, 1, NOW() - INTERVAL 1 HOUR, {4}, {5});", Data.BuildingID.barracks.ToString(), initializationData.accountID, 15, 20, 15, 20);
                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        command.ExecuteNonQuery();
                    }
                    query = String.Format("INSERT INTO buildings (global_id, account_id, x_position, y_position, level, track_time, x_war, y_war) VALUES('{0}', {1}, {2}, {3}, 1, NOW() - INTERVAL 1 HOUR, {4}, {5});", Data.BuildingID.armycamp.ToString(), initializationData.accountID, 15, 25, 15, 25);
                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        command.ExecuteNonQuery();
                    }
                        ...
                        using (MySqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                while (reader.Read())
                                {
                                    ...
                                    if (!TryParseBuildingID(reader["global_id"].ToString(), out building.id))
                                    {
                                        continue;
                                    }
                ...
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                if (!TryParseBuildingID(reader["global_id"].ToString(), out Data.BuildingID id))
                                {
                                    continue;
                                }
                ...
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                if (!TryParseBuildingID(reader["global_id"].ToString(), out Data.BuildingID buildingId))
                                {
                                    continue;
                                }
                ...
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                if (!TryParseBuildingID(reader["global_id"].ToString(), out Data.BuildingID global_id))
                                {
                                    continue;
                                }
                                switch (global_id)
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            ApplyDefensiveBuildingBalance(building);
        ...
        private static long _FindBattleTargetAsync(long account_id)
        {
            ...
            using (MySqlConnection connection = GetMysqlConnection())
            {
                string query = String.Format(
                    Terminal.allowOnlineBattleTargets
                        ? "SELECT id FROM accounts WHERE id <> {0} AND shield < NOW() ORDER BY RAND() LIMIT 1;"
                        : "SELECT id FROM accounts WHERE id <> {0} AND shield < NOW() AND is_online <= 0 ORDER BY RAND() LIMIT 1;",
                    account_id);
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    using (MySqlDataReader reader = command.ExecuteReader())
