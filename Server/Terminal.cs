...
namespace DevelopersHub.RealtimeNetworking.Server
{
    class Terminal
    {
        ...
        // Set to true during development to allow matching against online players' bases (simulates offline in production)
        public const bool allowOnlineBattleTargets = true;
        ...
        public static void OnClientConnected(int id, string ip)
        {
            if (!Server.clients[id].connected)
            {
                ...
                UpdateLog(1);
                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] Client #{id} connected from {ip}");
            }
        }
        ...
        public static void OnClientDisconnected(int id, string ip)
        {
            ...
            if (Server.clients[id].connected)
            {
                ...
                UpdateLog(-1);
                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] Client #{id} disconnected from {ip}");
            }
        }
